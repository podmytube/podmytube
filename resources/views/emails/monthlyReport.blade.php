...
@section('content')
    ...
    <p>
        Hello {{ $channel->user->firstname }} 👋
    </p>

    <p>
        Here is your <span class="bold">{{ $formattedPeriod }}</span> report for
        <strong>{{ $channel->channel_name }}</strong>
    </p>

    <p>
        To know what's coming next on Podmytube, you should take a look (and vote) on
        <a href="https://trello.com/b/g7YXh4OX/podmytube-roadmap" class="text-gray-900 underline" target="_blank">my public
            roadmap</a>
        <svg class="h-4 vamiddle" viewBox="0 0 256 256" version="1.1" xmlns="http://www.w3.org/2000/svg"
        ...
        or follow
        <a href="https://twitter.com/podmytube" class="text-gray-900 underline" target="_blank">@podmytube</a>
        <svg class="vamiddle" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns="http://www.w3.org/2000/svg"
            ...
            <g transform="translate(0 -1028.4)">
                <g>
                    ...
                    <path
                        ...
                        transform="translate(0 1028.4)" fill="#1ab2e8" />
                    <path fill="#1499ca"
        ...
        <table>
            ...
            <tbody>
                @foreach ($publishedMedias as $media)
                    <tr @if ($loop->even) style="background-color:#ccc" @endif>
                        <td style="text-align:left;">{{ $media->title }}</td>
                        <td style="text-align:center;">{{ $media->published_at->toFormattedDateString() }}</td>
                        <td style="text-align:center;">{{ $media->statusEmoji() }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    ...
    @if ($displayUpgradeMessage)
        <div style="text-align:center;">
            <p>
                <strong>
                    To take advantage of all of Podmytube's features and get all of your episodes included for an
                    unlimited period of time,
                    please consider upgrading your account. 🤗
                </strong>
            </p>
            <a href="{{ route('plans.index', $channel) }}" class="button">Upgrade</a>
        </div>
    @endif
@endsection
