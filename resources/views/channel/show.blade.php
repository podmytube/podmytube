    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-md-8">
                ...
                </span> : @if ($channel->created_at)
                    {{ $channel->created_at->format(__('localized.dateTimeFormat')) }}
                @endif
                <br />

                <span class="font-italic">
                    {{ __('messages.channel_podcast_updated_label') }}
                </span> : @if ($channel->podcast_updated_at)
                    {{ $channel->podcast_updated_at->format(__('localized.dateTimeFormat')) }}
                @else
                    {{ __('channels.never') }}
                @endif
            </div>
